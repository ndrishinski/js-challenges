...
function proteinSynthesis(dna) {
    let table = ['Ala', 'GCU', 'GCC', 'GCA', 'GCG', 'Leu', 'UUA', 'UUG', 'CUU', 'CUC', 'CUA', 'CUG', 'Arg', 'CGU', 'CGC', 'CGA', 'CGG', 'AGA', 'AGG', 'Lys', 'AAA', 'AAG', 'Asn', 'AAU', 'AAC', 'Met', 'AUG', 'Asp', 'GAU', 'GAC', 'Phe', 'UUU', 'UUC', 'Cys', 'UGU', 'UGC', 'Pro', 'CCU', 'CCC', 'CCA', 'CCG', 'Gln', 'CAA', 'CAG', 'Ser', 'UCU', 'UCC', 'UCA', 'UCG', 'AGU', 'AGC', 'Glu', 'GAA', 'GAG', 'Thr', 'ACU', 'ACC', 'ACA', 'ACG', 'Gly', 'GGU', 'GGC', 'GGA', 'GGG', 'Trp', 'UGG', 'His', 'CAU', 'CAC', 'Tyr', 'UAU', 'UAC', 'Ile', 'AUU', 'AUC', 'AUA', 'Val', 'GUU', 'GUC', 'GUA', 'GUG', 'Stop', 'UAG', 'UGA', 'UAA']
    let bigObj = {};
    let arr = '';
    let obj = {
      A: 'U',
    ...
    dna.split('').map((item, ind) => {
      let temp = ind += 1
      arr += obj[item]
      temp % 3 == 0  && ind != dna.length ? arr += ' ' : null
    })
  
  let selected = '';
  table.map(item => {
    item[1].toLowerCase() == item[1] ? selected = item : null
    bigObj[item] = selected
  })
  
  let chain = [];
  arr.split(' ').map(item => {
    item.length == 3 ? chain.push(bigObj[item]) : null
  })
  
    return [arr, chain.join(' ')]
  }